...
function Checkbox.new(parent, state)
    ...
    -- 创建选中标记（勾号）
    self.check_mark = lv.label_create(self.box)
    self.check_mark:set_text("✓")
    self.check_mark:set_style_text_color(parse_color(self.props.check_color), 0)
    self.check_mark:center()
    ...
    function self.set_property(self, name, value)
        ...
        elseif name == "check_color" then
            local col = parse_color(value)
            if self.check_mark then
                self.check_mark:set_style_text_color(col, 0)
            end
