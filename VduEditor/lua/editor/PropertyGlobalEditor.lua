-- PropertyGlobalEditor.lua
-- 全局组件属性编辑模块（如状态栏等不在画布上的组件）
local lv = require("lvgl")
local PropertyInputs = require("editor.PropertyInputs")

local PropertyGlobalEditor = {}

-- 创建元数据表（控件类型信息，只读）
function PropertyGlobalEditor.create_metadata_table(ctx, meta)
    local y_pos = 0
...
-- 创建实例名称编辑区域
function PropertyGlobalEditor.create_instance_name_editor(ctx, y_pos, global_entry)
    local instance = global_entry.instance
    ...
    PropertyInputs.create_text_input(ctx, "instance_name", current_name, false, global_entry, y_pos)
...
-- 创建属性编辑表（可修改）
function PropertyGlobalEditor.create_properties_table(ctx, y_pos, global_entry, meta)
    local instance = global_entry.instance
    ...
    for _, prop_def in ipairs(meta.properties) do
        ...
        if prop_type == "string" then
            ...
            if prop_def.multiline then
                ...
                local _, actual_height = PropertyInputs.create_multiline_text_input(ctx, prop_name, tostring(prop_value), is_read_only, global_entry, y_pos, lines)
                current_item_height = actual_height + 4
            else
                PropertyInputs.create_text_input(ctx, prop_name, tostring(prop_value), is_read_only, global_entry, y_pos)
            end
        elseif prop_type == "number" then
            PropertyInputs.create_number_input(ctx, prop_name, tonumber(prop_value) or 0, prop_def.min, prop_def.max, is_read_only, global_entry, y_pos)
        elseif prop_type == "boolean" then
            PropertyInputs.create_checkbox_input(ctx, prop_name, prop_value, is_read_only, global_entry, y_pos)
        elseif prop_type == "color" then
            PropertyInputs.create_color_input(ctx, prop_name, prop_value, is_read_only, global_entry, y_pos)
        elseif prop_type == "enum" then
            PropertyInputs.create_enum_dropdown(ctx, prop_name, prop_value, prop_def.options, is_read_only, global_entry, y_pos)
        else
...
-- 显示全局组件属性
function PropertyGlobalEditor.display_properties(ctx, global_entry)
    ctx:_clear_content_area()
    
    local instance = global_entry.instance
    local module = global_entry.module
    ...
    -- 1. 显示控件类型信息（只读）
    local y_pos = PropertyGlobalEditor.create_metadata_table(ctx, meta)
    
    -- 2. 显示实例名称编辑器
    y_pos = PropertyGlobalEditor.create_instance_name_editor(ctx, y_pos, global_entry)
    
    -- 3. 显示属性编辑
    y_pos = PropertyGlobalEditor.create_properties_table(ctx, y_pos, global_entry, meta)
    
    -- 4. 显示事件编辑器
    local PropertyEventEditor = require("editor.PropertyEventEditor")
    PropertyEventEditor.create_events_table(ctx, y_pos, global_entry, meta)
end

return PropertyGlobalEditor
